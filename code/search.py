...
def validate(valid_loader, model, epoch, cur_step, device, config, logger, writer, lam=None):
    top1 = utils.AverageMeter()
    losses = utils.AverageMeter()
    ...
    with torch.no_grad():
        for step, (X, y) in enumerate(valid_loader):
            ...
            # here we are to insert needed lambda
            if lam is None:
                logits = model(X)
            else:
                logits = model.forward(X, torch.tensor(lam).view(1, 1).to(device))
    ...
    logger.info(
        "Valid: [{:2d}/{}] Final Prec@1 {:.4%}; lambda = {:.1}".format(
            epoch + 1, config.epochs, top1.avg, lam))
